...
class DiscreteLSTM(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_size=(128, 256, 64), activation='relu'):
        super().__init__()
...
def main():
    ...
    network.to(device)
    network_optim = torch.optim.Adam(network.parameters(), lr=2e-4)
    ...
    for step in range(STEP):
        ...
        for epoch_i in range(5):
            ...
            for opt_i in range(int(EPISODE_NUM/OPTIM_BATCH)):
                ...
                #actor_loss_sum /= int(EPISODE_NUM/OPTIM_BATCH)
                #value_loss_sum /= int(EPISODE_NUM/OPTIM_BATCH)
                info = {
